...
core="$repo_root/target/wasm32-unknown-unknown/release/rintawa_package_manager.wasm"
...
cargo build --locked --release --target wasm32-unknown-unknown --manifest-path "$package_dir/Cargo.toml"
rm -rf "$output"
mkdir -p "$output"
