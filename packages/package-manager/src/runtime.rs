...
use crate::{
    ...
    model::{
        ...
        RegistryVersion, RepositoryPreference, merge_catalog, normalize_repositories,
        parse_registry, solve_install_plan, validate_repository_url, validate_rtw_url,
    },
};
...
impl InstalledStatus {
    fn message(self) -> Message {
        match self {
            ...
            Self::PublishedBuildDiffers => Message::StatusPublishedBuildDiffers,
...
fn dependency_info_for_locked(state: &ManagerState, subject: &str) -> (BTreeSet<String>, bool) {
    ...
    if let Some(release) = catalog_release_for_installed(state, installed)
        && installed
            .digest
            .eq_ignore_ascii_case(&release.artifact.sha256)
    {
        dependencies.extend(
            release
                .dependencies
...
fn install_selected_catalog_packages() -> Result<(), String> {
    ...
    let (selected, catalog, mut simulated_installed, disabled) = STATE.with(|state| {
        ...
        let simulated = installed_versions_for_solver(&state);
        let disabled = state
            .installed
            .iter()
...
fn install_catalog_package(package_id: &str, target: Option<&Version>) -> Result<(), String> {
    ...
    let (catalog, installed_versions) = STATE.with(|state| {
        let state = state.borrow();
        let versions = installed_versions_for_solver(&state);
...
fn download_and_import(selection: &crate::model::InstallSelection) -> Result<String, String> {
    ...
    verify_imported_artifact_digest(&selection.package_id, &artifact.sha256, &imported.digest)?;
...
fn verify_imported_artifact_digest(
    package_id: &str,
    expected: &str,
    actual: &str,
) -> Result<(), String> {
    if actual.eq_ignore_ascii_case(expected) {
        ...
        Err(format!(
            "{package_id} SHA-256 mismatch; repository metadata or downloaded RTW is inconsistent"
        ))
    }
}
...
fn installed_provider(state: &ManagerState, package: &InstalledPackage) -> String {
    let Some(catalog) = state.catalog.packages.get(&package.subject) else {
        return message(Message::LocalBaseline).to_string();
    };
    let is_published_build = catalog_release_for_installed(state, package).is_some_and(|release| {
        package
            .digest
            .eq_ignore_ascii_case(&release.artifact.sha256)
    });
    if is_published_build {
        repository_label(&catalog.repository_url)
    } else {
        message(Message::LocalUnpublishedBuild).to_string()
    }
}

fn catalog_release_for_installed<'a>(
...
fn installed_build_differs_from_catalog(state: &ManagerState, package: &InstalledPackage) -> bool {
    catalog_release_for_installed(state, package).is_some_and(|release| {
        !package
            .digest
            .eq_ignore_ascii_case(&release.artifact.sha256)
    })
...
fn installed_has_catalog_update(state: &ManagerState, package: &InstalledPackage) -> bool {
    let Some(installed_version) = package.version.as_ref() else {
        return false;
    };
    let Some(catalog_package) = state.catalog.packages.get(&package.subject) else {
        return false;
    };

    installed_version < &catalog_package.latest
        || (installed_version == &catalog_package.latest
            && installed_build_differs_from_catalog(state, package))
}

fn installed_versions_for_solver(state: &ManagerState) -> BTreeMap<String, Version> {
    state
        .installed
        .iter()
        .filter_map(|(id, package)| {
            if installed_build_differs_from_catalog(state, package) {
                None
            } else {
...
fn installed_status(state: &ManagerState, package: &InstalledPackage) -> InstalledStatus {
    ...
    if let Some(installed_version) = package.version.as_ref()
        && let Some(catalog_package) = state.catalog.packages.get(&package.subject)
    {
        if installed_version < &catalog_package.latest {
            return InstalledStatus::UpdateAvailable;
        }
        if installed_version == &catalog_package.latest
            && installed_build_differs_from_catalog(state, package)
        {
            return InstalledStatus::PublishedBuildDiffers;
        }
    }
        ...
        .filter(|package| {
            ...
            match state.catalog_filter {
                ...
                CatalogFilter::Updates => {
                    installed.is_some_and(|item| installed_has_catalog_update(state, item))
                }
            }
        })
        .collect::<Vec<_>>();
...
fn render_surface() {
    ...
    let rendered = STATE.with(|state| presentation::build_surface(&state.borrow()));
